...
export default function Nodes() {
    ...
    const resetNodeStates = useCallback((maybeInitialState) => {
    ...
    useEffect(() => {
        ...
        const offMutationsResource = mutationsResource.on((_id, resolution, rejection, version) => {
            if (mutations.version == null || mutations.version < version) {
                setMutations({ resolution, rejection, version });
                resetNodeStates();
            }
        });
        const offStateResource = stateResource.on((_id, resolution, rejection, version) => {
    ...
    if (mutations.rejection || initialState.rejection) {
        return (React.createElement("div", null,
            "Error: ",
            String((_a = mutations.rejection) !== null && _a !== void 0 ? _a : ''),
            " ",
            String((_b = initialState.rejection) !== null && _b !== void 0 ? _b : '')));
    }
    ...
    return (React.createElement("div", { className: "cs-group" },
        React.createElement("div", { className: "cs-controls" },
            React.createElement("button", { className: "cs-sync-state", onClick: syncNodes }, "Sync nodes!"),
            React.createElement("button", { className: "cs-sync-state", onClick: () => resetNodeStates() }, "Reset!")),
        Object.entries(nodeStates).map(([nodeName, nodeState]) => (React.createElement(Node, { key: nodeName, mutations: trackedMutations, name: nodeName, state: nodeState }))),
        React.createElement("div", { className: "cs-controls" },
            React.createElement("button", { className: "cs-sync-state", onClick: syncNodes }, "Sync nodes!"),
            React.createElement("button", { className: "cs-sync-state", onClick: () => resetNodeStates() }, "Reset!"))));
}
