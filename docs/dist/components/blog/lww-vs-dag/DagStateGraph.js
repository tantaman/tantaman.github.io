...
export default function DagStateGraph({ ctx, nodeName }) {
    const chart = useQuery(ctx, `SELECT
    ...
    FROM event_dag
    JOIN event ON event.id = event_dag.event_id`, [], (rows) => {
        ...
        for (const row of rows) {
            nodes.push({
                data: {
                    ...
                    label: `${(row.itemId || '').toString().slice(-4)}: [${row.type}, ${row.value}]`,
                },
            });
            if (row.parentId) {
                edges.push({
