...
function getColorForYear(year, backgrounds) {
    ...
    for (let i = 0; i < backgrounds.length - 1; i++) {
        if (year >= backgrounds[i].year && year <= backgrounds[i + 1].year) {
            const t = (year - backgrounds[i].year) /
...
function TimelineEra({ era, prevEndYear, pixelsPerYear, eraIndex }) {
    ...
    return (React.createElement("div", { style: { marginTop: marginTop + 'px', position: 'relative' } },
        ...
        era.events.map((event, i) => {
            ...
            return (React.createElement(FadeElement, { key: i, side: side },
                React.createElement(EventCard, { text: text, evidence: evidence, side: side })));
        }),
        era.note && (React.createElement(FadeElement, null,
            React.createElement(EraNoteCard, { note: era.note })))));
}
...
export default function Timeline({ eras = [], pixelsPerYear = DEFAULT_PX_PER_YEAR, backgrounds = [], }) {
    const containerRef = useRef(null);
    const [currentYear, setCurrentYear] = useState(eras.length > 0 ? eras[0].year : 2026);
    const [bgColor, setBgColor] = useState(backgrounds.length > 0 ? backgrounds[0].color : '#1a1a1a');
    const startYear = eras.length > 0 ? eras[0].year : 2026;
    const endYear = eras.length > 0
    ...
    return (React.createElement("div", { ref: containerRef, style: {
        ...
        React.createElement("div", { style: {
                ...
                margin: '0 auto',
                padding: '0 24px',
            } }, eras.map((era, i) => (React.createElement(TimelineEra, { key: i, era: era, prevEndYear: i > 0 ? eras[i - 1].endYear || eras[i - 1].year : null, pixelsPerYear: pixelsPerYear, eraIndex: i })))),
        React.createElement(YearIndicator, { year: currentYear })));
}
