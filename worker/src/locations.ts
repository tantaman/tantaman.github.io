...
export async function geocodeLocation(
  ...
  try {
    const url = `https://api.mapbox.com/search/searchbox/v1/forward?q=${encodeURIComponent(title)}&access_token=${mapboxToken}&limit=1&types=poi,address&country=us`;
...
export function extractLocations(body: string): LocationDef[] {
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      if (line.match(EVENT_RE) || line.match(/^#[tmbq]\s+/)) {
        current.description = descLines.join('\n').trim() || null;
        locations.push(current);
        current = null;
        descLines = [];
