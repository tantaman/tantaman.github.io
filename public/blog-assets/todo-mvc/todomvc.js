(function wrapper() {
  ...
  const todoApp = (state) => {
    ...
    return html`<div class="todoapp">
      ...
      <section
        class="main"
        ${state.items.length ? '' : 'style="display: none;"'}
      >
        ${toggleAll}
        <ul class="todo-list">
  ...
  window.updateFilter = (filter) => {
    let q = window.location.hash.lastIndexOf('?');
    if (q == -1) {
      q = window.location.hash.length;
    }
    window.location.hash = window.location.hash.substring(0, q) + '?' + filter;
  };
  // ## Events
  // ### On Create
  window.onCreate = (e) => {
