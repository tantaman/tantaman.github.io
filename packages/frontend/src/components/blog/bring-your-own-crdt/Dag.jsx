  ...
  return (
    <div>
      <div style={{ display: 'flex', height: 350, overflowY: 'scroll' }}>
        {ctxts.map((ctx, i) => (
          <section className="byo-todoapp" key={i}>
      ...
      <div>
        <center>
          <button className="byo-btn" disabled={syncing} onClick={syncNodes}>
            Sync Nodes &#128259;
          </button>
          <button
            className="byo-btn byo-btn-secondary"
            disabled={syncing}
            onClick={resetState}
          >
            Reset
...
const bareMutations = {
  async add(tx, todoId, content) {
    await tx.exec(
      `INSERT INTO todo (id, content, completed) VALUES (?, ?, ?)`,
      [todoId, content, 0],
    );
  },
...
function asTrackedMutations(mutations) {
  ...
  for (const [name, fn] of Object.entries(mutations)) {
    result[name] = async function (tx, ...args) {
      ...
      if (parents.length == 0) parents = [['ROOT']];
