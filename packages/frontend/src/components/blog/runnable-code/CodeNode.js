...
export class CodeNode {
  constructor() {
    ...
    this.onResourceReady = (id, resolution, rejection, version) => {
      ...
      for (const [u, v] of this.uses) {
        ...
        if (v < resource.version) seenAllVersions = false;
      }

      if (!allReady) return;
      if (!seenAllVersions) this.run(this.body);
    };
  }
  ...
  eval(code) {
    ...
    for (const u of noLongerUses) {
      getResource(u)?.off(this.onResourceReady);
    }

    this.uses = new Map();
    for (const u of uses) this.uses.set(u, 0);
    this.provides = provides;

    for (const p of this.provides) {
      getResource(p)?.invalidate();
    }

    if (this.uses.size == 0) this.run(body);

    for (const u of this.uses.keys()) {
      getResource(u)?.on(this.onResourceReady);
    }
  }
  ...
  run(body) {
    ...
    for (const p of this.provides) {
      ...
      const index = i;
      resource?.setPromise(result.then((r) => r[index]));
      ++i;
    }
    ...
    result.then(
      (r) => {
        if (this.pendingPromise != result) return;
        if (isPromise(r[0])) {
  ...
  waitingFor() {
    return [...this.uses.keys()].filter((u) => !getResource(u)?.isReady);
  }
