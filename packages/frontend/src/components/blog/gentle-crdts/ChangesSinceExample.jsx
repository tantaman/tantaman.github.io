...
function merge(to, from, since) {
  ...
  fromRows.forEach((row) => {
    if (row.local_row_time > max) max = row.local_row_time;
...
export default function ChangesSinceExample() {
  ...
  function modifyRow() {
    setState((state) => {
      const next = structuredClone(state);
      if (next[2].rows.length === 0) return next;
      const time = ++next[2].localTime;
  ...
  return (
    <div>
      ...
      <div className="gc-tbl-contain">
        ...
        <button className="gc-btn" onClick={mbSince}>
      ...
      <div className="gc-tbl-contain">
        ...
        <button className="gc-btn" onClick={mcSince}>
      ...
      <div className="gc-tbl-contain">
        ...
        <button className="gc-btn" style={{ marginRight: 4 }} onClick={addRow}>
          Add Row
        </button>
        <button className="gc-btn" onClick={modifyRow}>
          Modify Row
        </button>
      </div>
    </div>
