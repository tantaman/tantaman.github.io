...
- `pnpm upload-posts-manifest` - Upload `docs/posts-manifest.json` to Cloudflare R2 (`tantaman-site/posts-manifest.json`). Like the search index, the manifest is **not committed** — it is generated by `pnpm build` (gitignored locally for dev) and served by the worker at `tantaman.com/posts-manifest.json` from `SITE_BUCKET`. Run this after a content build to publish (the client tags/thoughts UIs and the worker's now/digest/typeahead/mcp/ig-card modules all read it).
