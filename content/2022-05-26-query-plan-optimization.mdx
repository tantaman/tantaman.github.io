...
tags: [software]
concern: [systems]
description: 'Aphrodite''s query optimizer aims to minimize database calls and reduce returned data by hoisting operations like filters, joins, and limits into database queries rather than handling them in the application layer, and by collapsing derived expressions and hop plans to their source equivalents. This optimization approach is particularly important for ORMs to avoid common performance pitfalls like the N+1 problem and becomes even more critical when queries span multiple data sources.'
---
