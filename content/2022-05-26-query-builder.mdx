...
As a JSON structure it looks like:
...
Running each node in the list from the end back to the start will run the full query. In reality, however, we convert the query to a `plan` and optimize the `plan` before running it.
See [Query Planning](./2022-05-26-query-planning) for more info on this.

The structure is a bit more complicated in practice since we need to constrain queries to only fetch items related to the prior hop but this is the basic idea.
